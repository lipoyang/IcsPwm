...
INTERRUPT void uart_tx_interrupt(void)
{
    if (Serial._tx_buffer_head != Serial._tx_buffer_tail) {
    	...
    	URT_TXDx = Serial._tx_buffer[Serial._tx_buffer_tail];
    	Serial._tx_buffer_tail = (Serial._tx_buffer_tail + 1) % Serial._tx_buf_size;
    }
}
#if ( UART1_CHANNEL == 1 )
extern "C"
INTERRUPT void uart1_tx_interrupt(void)
{
    if (Serial1._tx_buffer_head != Serial1._tx_buffer_tail) {
        // There is more data in the output buffer. Send the next byte
    	URT1_TXDx = Serial1._tx_buffer[Serial1._tx_buffer_tail];
    	Serial1._tx_buffer_tail = (Serial1._tx_buffer_tail + 1) % Serial1._tx_buf_size;
    }
}
#endif
#if ( UART2_CHANNEL == 2 )
...
INTERRUPT void uart2_tx_interrupt(void)
{
    if (Serial2._tx_buffer_head != Serial2._tx_buffer_tail) {
        // There is more data in the output buffer. Send the next byte
    	URT2_TXDx = Serial2._tx_buffer[Serial2._tx_buffer_tail];
    	Serial2._tx_buffer_tail = (Serial2._tx_buffer_tail + 1) % Serial2._tx_buf_size;
    }
}
#endif
#else
...
void HardwareSerial::begin(unsigned long baud, unsigned short config){
...
void HardwareSerial::begin(unsigned long baud, unsigned short config, int rx_buf, int tx_buf)
{
